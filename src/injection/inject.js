...
window.finishJob = () => {
  const current = document.querySelector(".ncells .currents").parentElement
    .parentElement;
  const nextCellIndex = alleclls.findIndex(item => {
    return (
      item.children[0].children[0].id === current.children[0].children[0].id
    );
  });
...
window.getTeacherAjax = (courseId, clazzid, chapterId) => {
  ...
  document.getElementById("mainid").innerHTML =
    '<div style="width:32px;height:32px;margin:0 auto;padding:300px 0"><img src="/images/courselist/loading.gif" /></div>';
  jQuery.post(
    "/mycourse/studentstudyAjax",
    {
      courseId: courseId,
      clazzid: clazzid,
      chapterId: chapterId
